...
namespace WTERP
{
    public partial class Form1C : Form
    {
        ...
        public void LoadData()
        {
            ...
            {
                string SQL = "select TOP 200 PROD.K_NO, KIND.K_NAME, P_NO, P_NAME, P_NAME1, P_NAME3, P_NAME2, UNIT, TRANS, BUNIT, CUNIT, PRICE, P_WEG, QTYKG, QTYSTORE, QTYPIC, BASEDATE, " +
                    " QTYFT, INDATE, QTYORD, OUTDATE, QTYBAT, QTYPROD, MEMO1, MEMO2, MEMO3, MEMO4, COST, COST_NEW, COST_AVG, PM_DATE, TN_DATE, DY_DATE, GD_DATE, TN_DATE1, DY_DATE1, GD_DATE1, " +
                    "PT_DATE, PT_FT, PK_DATE,PROD.USR_NAME from PROD full outer join KIND on PROD.K_NO = KIND.K_NO";
                datatable = conn.readdata(SQL);
                foreach (DataRow dr in datatable.Rows)
                {
                    dr["QTYKG"] = conn.FormatNumber(dr["QTYKG"].ToString());
                    dr["QTYPIC"] = conn.FormatNumber(dr["QTYPIC"].ToString());
                    dr["QTYSTORE"] = conn.FormatNumber(dr["QTYSTORE"].ToString());
                    dr["QTYFT"] = conn.FormatNumber(dr["QTYFT"].ToString());
                    dr["QTYORD"] = conn.FormatNumber(dr["QTYORD"].ToString());
        ...
        public void hienthidata()
        {
            ...
            tb40.Text = currenRow["COST"].ToString();
            tb41.Text = currenRow["COST_NEW"].ToString();
            tb42.Text = currenRow["COST_AVG"].ToString();
            tb43.Text = (float.Parse(currenRow["COST_NEW"].ToString()) * float.Parse(currenRow["QTYSTORE"].ToString())).ToString(); ;
        ...
        private void f6ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            this.tb40.Enabled = true;
            this.tb41.Enabled = true;
            this.tb42.Enabled = true;
        ...
        private void btok_Click(object sender, EventArgs e)
        {
            ...
            if (f2ToolStripMenuItem.Checked == true)
            {
                ...
                con.Open();
                string st1 = "insert into dbo.PROD(K_NO, P_NO, P_NAME, P_NAME1, P_NAME3, P_NAME2, UNIT, TRANS, BUNIT, CUNIT, PRICE, P_WEG, QTYKG, QTYSTORE, QTYPIC, BASEDATE, QTYFT, INDATE, QTYORD, OUTDATE, QTYBAT, QTYPROD, MEMO1, MEMO2, MEMO3, MEMO4, COST, COST_NEW, COST_AVG, PM_DATE, TN_DATE, DY_DATE, GD_DATE, TN_DATE1, DY_DATE1, GD_DATE1, PT_DATE, PT_FT, PK_DATE,USR_NAME) values" +
                    "(@K_NO, @P_NO, @P_NAME, @P_NAME1, @P_NAME3, @P_NAME2, @UNIT, @TRANS, @BUNIT, @CUNIT, @PRICE, @P_WEG, @QTYKG, @QTYSTORE, @QTYPIC, @BASEDATE, @QTYFT, @INDATE, @QTYORD, @OUTDATE, @QTYBAT, @QTYPROD, @MEMO1, @MEMO2, @MEMO3, @MEMO4, @COST, @COST_NEW, @COST_AVG, @PM_DATE, @TN_DATE, @DY_DATE, @GD_DATE, @TN_DATE1, @DY_DATE1, @GD_DATE1, @PT_DATE, @PT_FT, @PK_DATE,@USR_NAME)";
                ...
                cm.Parameters.AddWithValue("@COST", float.Parse(tb40.Text));
                cm.Parameters.AddWithValue("@COST_NEW", float.Parse(tb41.Text));
                cm.Parameters.AddWithValue("@COST_AVG", float.Parse(tb42.Text));
                //cm.Parameters.AddWithValue("@", tb43.Text);
        ...
        private void tb26_MouseDoubleClick(object sender, MouseEventArgs e)
        {
            ...
            string dl1 = FormSearchLeather2.ID.M_NAME;
            if (dl1 != string.Empty)
                Get_tb1();
