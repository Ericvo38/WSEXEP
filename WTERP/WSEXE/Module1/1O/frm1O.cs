...
namespace WTERP
{
    public partial class Form1O : Form
    {
        ...
        #region Load Dataa
        private void Form1Oquanlidulieudoday_Load(object sender, EventArgs e)
        {
        ...
        public void LoadData()
        {
            ...
            cm.CommandText = "select T_NO, T_NAME,USR_NAME from THICK";
            ...
            dataGridViewForm1O.DataSource = bindingsource;

            this.dataGridViewForm1O.Columns["USR_NAME"].Visible = false;
        }

        public void hienthi()
        {
            tb1.Text = dataGridViewForm1O.Rows[0].Cells["T_NO"].Value.ToString();
            tb2.Text = dataGridViewForm1O.Rows[0].Cells["T_NAME"].Value.ToString();
            lblEditBy.Text = dataGridViewForm1O.Rows[0].Cells["USR_NAME"].Value.ToString();
        }

        public void hienthi2()
        {
            this.tb1.Text = dataGridViewForm1O.Rows[dataGridViewForm1O.CurrentRow.Index].Cells[0].Value.ToString();
            this.tb2.Text = dataGridViewForm1O.Rows[dataGridViewForm1O.CurrentRow.Index].Cells[1].Value.ToString();
            this.lblEditBy.Text = dataGridViewForm1O.Rows[dataGridViewForm1O.CurrentRow.Index].Cells[2].Value.ToString();
        }

        private void btdau_Click(object sender, EventArgs e)
        {
            dataGridViewForm1O.ClearSelection();
            dataGridViewForm1O.Rows[0].Selected = true;
            // bindingsource = new BindingSource();
            bindingsource.DataSource = datatable;
            dataGridViewForm1O.DataSource = bindingsource;
        ...
        private void bttruoc_Click(object sender, EventArgs e)
        {
            try
            {
                int IndexNow = dataGridViewForm1O.CurrentRow.Index;
                if (dataGridViewForm1O.Rows.Count > IndexNow)
                {
                    dataGridViewForm1O.Rows[IndexNow - 1].Selected = true;
                }
        ...
        {
            dataGridViewForm1O.ClearSelection();
            int so = dataGridViewForm1O.Rows.Count - 1;
            //MessageBox.Show(so.ToString());
            dataGridViewForm1O.Rows[so - 1].Selected = true;
            bindingsource.DataSource = datatable;
            dataGridViewForm1O.DataSource = bindingsource;
        ...
        private void dataGridViewForm1O_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            hienthi2();
        }
        #endregion
        ...
        public void checkNofication()
        {
            ...
            {
                txtText = "Bạn không thể sửa mã độ dày";
                txtThongBao = "Thông Báo";
                txtText1 = "Bạn Cần Thay Đổi Mã Độ Dày Với 4 Ký Tự";
                txtText2 = "Mã Độ Dày Đã Tồn Tại";
                txtText3 = "Bạn chưa nhập mã độ dày";
                txtText4 = "Bạn chưa nhập tên độ dày";
                txtText5 = "Mã Độ Dài Đã Tồn Tại \n Bạn Vui Lòng Nhấn OK, [Đóng] và Thao Tác Lại Nhé";
            ...
            if (DataProvider.LG.rdVietNam == true)
            {
                ...
                txtText1 = "Bạn Cần Thay Đổi Mã Độ Dày Với 4 Ký Tự";
                txtText2 = "Mã Độ Dày Đã Tồn Tại";
                txtText3 = "Bạn chưa nhập tên độ dày";
                txtText4 = "Bạn chưa nhập tên độ dày";
                txtText5 = "Mã Độ Dài Đã Tồn Tại \n Bạn Vui Lòng Nhấn OK, [Đóng] và Thao Tác Lại Nhé";
                txtText6 = "Xóa Thành Công";
                txtLuuTC = "Lưu Thành Công";
            ...
            {
                txtText = "You cannot edit the thickness code";
                txtThongBao = "Nofication";
                txtText1 = "You Need to Change the Thickness Code With 4 Characters";
                txtText2 = "Thickness Code Exists";
                txtText3 = "You have not entered the thickness code";
                txtText4 = "You have not entered the thickness name";
                txtText5 = "Length Code Exists \n Please Click OK, [Close] and Re-Operate";
        ...
        private void btok_Click(object sender, EventArgs e)
        {
            ...
            {
                string a = tb1.Text;
                string SQL = "select T_NO from THICK where T_NO = '" + a + "'";
                DataTable dt = conn.readdata(SQL);

                foreach (DataRow dr in dt.Rows)
                {
                    if (a.ToString() == dr["T_NO"].ToString())
                    {
                ...
                con.Open();
                string st1 = "insert into dbo.THICK(T_NO, T_NAME,USR_NAME) values(@T_NO, @T_NAME,@USR_NAME)";
                SqlCommand cm = new SqlCommand(st1, con);

                cm.Parameters.AddWithValue("@T_NO", tb1.Text);
            ...
            else if (f3ToolStripMenuItem.Checked == true)
            {
                ...
                if (dr == DialogResult.Yes)
                {
                    try
                    {
                        ...
                        con.Open();
                        cm = con.CreateCommand();
                        cm.CommandText = "delete from THICK where T_NO ='" + Madodai + "'";
                        cm.ExecuteNonQuery();
            ...
            else if (f4ToolStripMenuItem.Checked == true)
            {
                ...
                con = new SqlConnection(st);
                con.Open();
                string st1 = "update dbo.THICK set T_NO = @T_NO, T_NAME = @T_NAME,USR_NAME = @USR_NAME where T_NO = @T_NO";
                SqlCommand cm = new SqlCommand(st1, con);
